...
当前版本不会直接改写源码，也不会替代 `moon test`。它更关注发布前
的结构性信号：哪些包没有测试、测试文件分布是否合理、公共接口是否
缺少测试信号，以及哪些代码位置适合作为后续变异测试目标。
...
- 后续工程演进：为变异测试、质量趋势报告和 CI 集成提供基础数据。
...
- 执行默认质量门禁：项目测试数、包级测试、README、许可证声明和
  CI workflow。
...
项目首版聚焦“分析和门禁”，不做在线平台，不引入外部依赖，也不把边
界扩展到完整覆盖率系统。这种取舍保证了项目可构建、可运行、可测试，
同时给后续的自动变异执行和质量趋势分析保留扩展空间。
